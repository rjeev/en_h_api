...
app.post('/social/linkedin/headline', async (req, res, next) => {

	try {
			let { headline, description, audience, tone, n } = req.body
		
			// append the word hello before the start of the prompt text
			let prompt = `Create a long, engaging LinkedIn ad headline based on metadata provided:\n###\n` +
			...
			`TITLE: Interview 101 Course\n` + 
			`${description ? `DESCRIPTION: From answering questions to writing cover letters, I’ll teach you to get your next job in this new class.\n` : ``}` + 
			`${audience ? `AUDIENCE: Job seekers\n` : ``}` + 
			`${tone ? `TONE: Helpful Knowledge\n` : ``}` + 
			`HEADLINE: Secrets & Skills to Interview Success\n` + 
			`###\n` 
  ...
  app.post('/social/linkedin/description', async (req, res, next) => {

	try {
			let { headline, description, audience, tone, n } = req.body
		
			// append the word hello before the start of the prompt text
			let prompt = `Create a short, engaging LinkedIn ad description based on metadata provided:\n###\n` +
			...
			`TITLE: Interview 101 Course\n` + 
			`${description ? `DETAILS: Secrets & Skills to Interview Success\n` : ``}` + 
			`${audience ? `AUDIENCE: Job seekers\n` : ``}` + 
			`${tone ? `TONE: Helpful Knowledge\n` : ``}` + 
			`DESCRIPTION: From answering questions in interviews to writing cover letters, I’ll teach you how you need to know to get your next job.\n` + 
			`###\n` 
  ...
  app.post('/social/linkedin/keywords', async (req, res, next) => {

	try {
			let { headline, description, audience, } = req.body
		
			// append the word hello before the start of the prompt text
			let prompt = `Brainstorm dozens of keywords for a LinkedIn Ad Campaign based on metadata provided:\n###\n` +
